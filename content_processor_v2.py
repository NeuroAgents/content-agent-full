#!/usr/bin/env python
# content_processor_v2.py - Микросервис для обработки и перевода контента
# с использованием существующей структуры базы данных
...
class ContentProcessor:
    ...
    def __init__(self):
        ...
        # Создаем генеративную модель (используем актуальное название модели)
    ...
    def update_article(self, article_id: str, result: ProcessingResult) -> bool:
        ...
        try:
            # Создаем JSON-структуру для хранения переводов
            translations = {
                "ru": {
            ...
            self.db_client.table('content_items') \
                .update({
                    ...
                    'keywords': [translations["ru"]["title"]] if translations["ru"]["title"] else [],
                    'category': 'translated',
                    'content': json.dumps(metadata),  # Сохраняем JSON в поле content
                    'is_translated': True
                }) \
                .eq('id', article_id) \
                .execute()
    ...
    def process_article(self, article: Dict[str, Any]) -> ProcessingResult:
        ...
        try:
            # Сначала проверяем, может ли контент быть уже в формате JSON
            try:
                # Пытаемся распарсить JSON
                json_content = json.loads(content)
                
                # Если это JSON с переводами, пропускаем статью
                if isinstance(json_content, dict) and "translations" in json_content:
                    logger.info(f"Статья {article_id} уже содержит переводы, пропускаем")
                    return ProcessingResult(
                        success=False,
                        error_message=f"Статья {article_id} уже содержит переводы"
                    )
                
                # Если это JSON, но не с переводами, извлекаем исходный контент
                if isinstance(json_content, dict) and "content" in json_content:
                    original_content = json_content.get("content", "")
                    if original_content:
                        content = original_content
            except (json.JSONDecodeError, TypeError):
                # Если не JSON, используем контент как есть
                pass
